...
- **Exemplar re-ID** — the verified detection bootstraps an instance-level exemplar bank (DINOv2 features over views harvested from the SAM2 track); the bank sweeps the whole CLIP index and recovers every other occurrence of the *same* object, each verified by instance similarity and saved as an annotated thumbnail. Found occurrences feed back into the presence score.
- **Machine-readable export** — `--json-out` writes the full result: query plan, evidence vector, presence score, per-instance tracks with COCO RLE masks, re-ID occurrences, per-stage timing, and config+git provenance hashes.
