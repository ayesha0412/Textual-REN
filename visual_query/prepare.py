...
def create_feature_dict(item, data_dir):
    ...
    fps = item['clip_fps']
    # Use pre-cut clips instead of full_scale videos — timestamps start from 0 within each clip
    clip_file = os.path.join(data_dir, f'v2/clips/{clip_uid}.mp4')
    full_scale_file = os.path.join(data_dir, f'v2/full_scale/{video_uid}.mp4')
    if os.path.exists(clip_file):
        video_file = clip_file
        clip_duration = item['video_end_sec'] - item['video_start_sec']
        timesteps = np.arange(0.0, clip_duration, step=1.0 / fps)
    else:
        video_file = full_scale_file
